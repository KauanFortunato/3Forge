    ...
    objects: {
      ...
      items: {
        ...
        properties: {
          ...
          color: { type: "string" },
          opacity: { type: "number" },
          materialType: { type: ["string", "null"], enum: ["basic", "standard", "physical", "toon", "lambert", "phong", "normal", "depth", null] },
          side: { type: ["string", "null"], enum: ["front", "back", "double", null] },
          mapImageId: { type: ["string", "null"] },
          emissive: { type: ["string", "null"] },
          emissiveIntensity: { type: ["number", "null"] },
          roughness: { type: ["number", "null"] },
          metalness: { type: ["number", "null"] },
          transmission: { type: ["number", "null"] },
          thickness: { type: ["number", "null"] },
          clearcoat: { type: ["number", "null"] },
          clearcoatRoughness: { type: ["number", "null"] },
...
export function createBlueprintFromAiScene(spec: AiSceneSpec, options: { baseBlueprint?: ComponentBlueprint } = {}): ComponentBlueprint {
  ...
  return {
    ...
    fonts: options.baseBlueprint ? options.baseBlueprint.fonts.map((font) => ({ ...font })) : [],
...
export function createAiBlueprintResult(
  sceneSpec: AiSceneSpec,
  rawText = JSON.stringify(sceneSpec, null, 2),
  executedModel?: string,
  ...
  return {
    ...
    objects: blueprint.nodes.flatMap((node): AiPrimitiveSpec[] => {
      ...
      const createBase = (type: AiPrimitiveType) => ({
        ...
        mapImageId: node.material.mapImageId ?? null,
...
function applyMaterial(node: EditorNode, spec: AiPrimitiveSpec): void {
  if (node.type === "group") {
    return;
  }

  const materialType = isMaterialType(spec.materialType) ? spec.materialType : "standard";
  const material = createMaterialSpec(normalizeColor(spec.color), materialType);
  material.opacity = clampNumber(spec.opacity, 0.05, 1, 1);
  material.transparent = material.opacity < 1;
  material.side = isMaterialSide(spec.side) ? spec.side : node.type === "plane" ? "double" : "front";
  material.emissive = normalizeColor(spec.emissive, material.emissive);
  material.emissiveIntensity = clampNumber(spec.emissiveIntensity, 0, 10, material.emissiveIntensity);
  material.roughness = clampNumber(spec.roughness, 0, 1, material.roughness);
  material.metalness = clampNumber(spec.metalness, 0, 1, material.metalness);
  material.transmission = clampNumber(spec.transmission, 0, 1, material.transmission);
  material.thickness = clampNumber(spec.thickness, 0, 10, material.thickness);
  material.clearcoat = clampNumber(spec.clearcoat, 0, 1, material.clearcoat);
  material.clearcoatRoughness = clampNumber(spec.clearcoatRoughness, 0, 1, material.clearcoatRoughness);

  if (typeof spec.mapImageId === "string" && spec.mapImageId.trim()) {
    material.mapImageId = spec.mapImageId.trim();
  }
...
function normalizeColor(value: unknown, fallback = "#8df0ff"): string {
