...
Every object has material fields. Keep them simple unless the user asks for a specific look.

- `materialType`: use `standard` for most objects.
- Use `basic` for flat labels, UI-like panels, icons, and unlit color.
- Use `physical` only for glass, clear plastic, polished metal, or glossy coated surfaces.
- Use `toon` for cartoon/stylized models.
- Use `lambert` or `phong` only when the user asks for older/simple shading.
- Use `normal` or `depth` only when the user explicitly asks for technical preview materials.
- `side`: use `front` for normal solids, `double` for planes, signs, screens, decals, cards, thin panels, or anything that must be visible from both sides, and `back` only when the back face should render.
- `mapImageId`: reusable texture image ID from the current blueprint image assets. Use it only when an existing image ID is available in the current scene/context. Never invent IDs, base64, file paths, or URLs. Use `null` otherwise.
- `emissive` and `emissiveIntensity`: use for lights, glows, screens, and accents. Use `null` when not needed.
- `roughness` and `metalness`: use with `standard` or `physical`. Lower roughness is glossier; higher metalness reads as metal.
- `transmission`, `thickness`, `clearcoat`, and `clearcoatRoughness`: use only with `physical` materials for glass/plastic/coated surfaces. Use `null` otherwise.
